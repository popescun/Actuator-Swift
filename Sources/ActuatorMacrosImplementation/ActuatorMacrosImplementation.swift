...
public struct ActuatorWithParameters: DeclarationMacro {
  public static func expansion(
    ...
    guard var arrayElements = node.argumentList.first?.expression.as(ArrayExprSyntax.self)?.elements else
    {
    ...
    guard let returnElement = arrayElements[startIndex].as(ArrayElementSyntax.self)?.expression.as(StringLiteralExprSyntax.self)?.representedLiteralValue else {
      throw CocoaError(.featureUnsupported) // You might want to replace this error with a more descriptive replacement
    }
    ...
    for element in arrayElements {
      guard let argument = element.as(ArrayElementSyntax.self)?.expression.as(StringLiteralExprSyntax.self)?.representedLiteralValue else {
        throw CocoaError(.featureUnsupported) // You might want to replace this error with a more descriptive replacement
      }
...
public struct Actuator: DeclarationMacro {
  public static func expansion(
    ...
    guard var arg = node.argumentList.first?.expression.as(IntegerLiteralExprSyntax.self)?.literal.text else
    {
      NSException(name: .genericException, reason: "Failed to get the macro's argument!").raise()
      return[]
    }
    ...
    if paramCount == 0 {
      NSException(name: .genericException, reason: "paramCount is 0!").raise()
    }
